...
4. The workflow assembles `deploy_pkg/` = `static/index.html` + `static/toolkit.html` + `static/projects.html` + `static/welders.html` + `static/toolkit.bundle.js` + `documents.html` + a `CNAME` for `opcoportal.com`, then publishes it to the `gh-pages` branch via `peaceiris/actions-gh-pages`. GitHub Pages serves `gh-pages` at the custom domain. It also still commits the regenerated root `index.html` back to `main` as a git-history backup.
